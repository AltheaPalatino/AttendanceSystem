...
class UserModel extends Database {
    ...
    public function getById($id) {
        $stmt = $this->pdo->prepare("SELECT u.*, c.name as course_name, c.code as course_code 
                                     FROM users u 
    ...
    public function allStudents() {
        $stmt = $this->pdo->prepare("SELECT u.*, c.code as course_code, c.name as course_name 
                                     FROM users u 
                                     LEFT JOIN courses c ON u.course_id = c.id 
...
class CourseModel extends Database {
    public function createCourse($data) {
        $sql = "INSERT INTO courses (code, name, start_time, late_grace_minutes) 
                VALUES (:code, :name, :start_time, :late_grace_minutes)";
        $stmt = $this->pdo->prepare($sql);
        return $stmt->execute([
            ':code' => $data['code'],
            ':name' => $data['name'],
    ...
    public function updateCourse($id, $data) {
        $sql = "UPDATE courses 
                SET code = :code, name = :name, start_time = :start_time, late_grace_minutes = :late_grace_minutes 
                WHERE id = :id";
        $stmt = $this->pdo->prepare($sql);
        return $stmt->execute([
            ':code' => $data['code'],
...
class AttendanceModel extends Database {
    ...
    public function getHistoryByUser($user_id) {
        $stmt = $this->pdo->prepare("SELECT a.*, c.code as course_code, c.name as course_name 
                                     FROM attendance a 
    ...
    public function getByCourseAndYear($course_id, $year_level) {
        $stmt = $this->pdo->prepare("SELECT a.*, u.name as student_name, u.email 
                                     FROM attendance a 
                                     JOIN users u ON a.user_id = u.id 
                                     WHERE a.course_id = :course_id AND a.year_level = :year_level 
                                     ORDER BY a.filed_at DESC");
        $stmt->execute([':course_id' => $course_id, ':year_level' => $year_level]);
        return $stmt->fetchAll();
    }

    public function markChecked($attendance_id, $remarks = null) {
        $sql = "UPDATE attendance SET status = 'checked', remarks = :remarks WHERE id = :id";
        $stmt = $this->pdo->prepare($sql);
        return $stmt->execute([
            ':remarks' => $remarks,
            ':id' => $attendance_id
